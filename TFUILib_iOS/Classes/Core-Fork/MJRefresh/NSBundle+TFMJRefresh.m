...
@implementation NSBundle (TFMJRefresh)
...
#else
        NSBundle *containnerBundle = [NSBundle bundleForClass:[TFMJRefreshComponent class]];
#endif
...
+ (UIImage *)tf_mj_arrowImage
{
    ...
    if (arrowImage == nil) {
        arrowImage = [[UIImage imageWithContentsOfFile:[[self tf_mj_refreshBundle] pathForResource:@"arrow@2x" ofType:@"png"]] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
    }
    return arrowImage;
}
...
+ (UIImage *)tf_mj_trailArrowImage {
    static UIImage *arrowImage = nil;
    if (arrowImage == nil) {
        arrowImage = [[UIImage imageWithContentsOfFile:[[self tf_mj_refreshBundle] pathForResource:@"trail_arrow@2x" ofType:@"png"]] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
    }
    return arrowImage;
}
...
{
    return [self tf_mj_localizedStringForKey:key value:nil];
}

+ (NSString *)tf_mj_localizedStringForKey:(NSString *)key value:(NSString *)value
{
    NSString *table = TFMJRefreshConfig.defaultConfig.i18nFilename;
    
    // 如果没有缓存, 则走初始化逻辑
    if (mj_defaultI18nBundle == nil) {
        NSString *language = TFMJRefreshConfig.defaultConfig.languageCode;
        // 如果配置中没有配置语言
        if (!language) {
            language = [NSLocale preferredLanguages].firstObject;
        }
        NSBundle *bundle = TFMJRefreshConfig.defaultConfig.i18nBundle;
...
+ (NSBundle *)mj_defaultI18nBundleWithLanguage:(NSString *)language {
    ...
    return [NSBundle bundleWithPath:[[NSBundle tf_mj_refreshBundle] pathForResource:language ofType:@"lproj"]];
}
@end

@implementation TFMJRefreshConfig (Bundle)
