...
- (void)scrollViewContentOffsetDidChange:(NSDictionary *)change {
    ...
    // 当前的contentOffset
    CGFloat currentOffsetX = self.scrollView.tf_mj_offsetX;
    // 尾部控件刚好出现的offsetX
    CGFloat happenOffsetX = [self happenOffsetX];
    // 如果是向右滚动到看不见右边控件，直接返回
    if (currentOffsetX <= happenOffsetX) return;
    
    CGFloat pullingPercent = (currentOffsetX - happenOffsetX) / self.tf_mj_w;
    ...
    if (self.scrollView.isDragging) {
        self.pullingPercent = pullingPercent;
        // 普通 和 即将刷新 的临界点
        CGFloat normal2pullingOffsetX = happenOffsetX + self.tf_mj_w;
        
        if (self.state == TFMJRefreshStateIdle && currentOffsetX > normal2pullingOffsetX) {
            self.state = TFMJRefreshStatePulling;
        } else if (self.state == TFMJRefreshStatePulling && currentOffsetX <= normal2pullingOffsetX) {
            // 转为普通状态
    ...
    if (state == TFMJRefreshStateNoMoreData || state == TFMJRefreshStateIdle) {
        ...
        if (TFMJRefreshStateRefreshing == oldState) {
            [UIView animateWithDuration:self.slowAnimationDuration animations:^{
                ...
                self.scrollView.tf_mj_insetR -= self.lastRightDelta;
                // 自动调整透明度
        ...
        // 刚刷新完毕
        if (TFMJRefreshStateRefreshing == oldState && deltaW > 0 && self.scrollView.mj_totalDataCount != self.lastRefreshCount) {
            self.scrollView.tf_mj_offsetX = self.scrollView.tf_mj_offsetX;
        }
        ...
        [UIView animateWithDuration:self.fastAnimationDuration animations:^{
            CGFloat right = self.tf_mj_w + self.scrollViewOriginalInset.right;
            CGFloat deltaW = [self widthForContentBreakView];
            if (deltaW < 0) { // 如果内容宽度小于view的宽度
                right -= deltaW;
            }
            self.lastRightDelta = right - self.scrollView.tf_mj_insetR;
            self.scrollView.tf_mj_insetR = right;
            
            // 设置滚动位置
            CGPoint offset = self.scrollView.contentOffset;
            offset.x = [self happenOffsetX] + self.tf_mj_w;
...
- (void)scrollViewContentSizeDidChange:(NSDictionary *)change {
    ...
    // 内容的宽度
    CGFloat contentWidth = self.scrollView.tf_mj_contentW + self.ignoredScrollViewContentInsetRight;
    // 表格的宽度
    CGFloat scrollWidth = self.scrollView.tf_mj_w - self.scrollViewOriginalInset.left - self.scrollViewOriginalInset.right + self.ignoredScrollViewContentInsetRight;
    // 设置位置和尺寸
    self.tf_mj_x = MAX(contentWidth, scrollWidth);
}

- (void)placeSubviews {
    [super placeSubviews];
    
    self.tf_mj_h = _scrollView.tf_mj_h;
    // 设置自己的宽度
    self.tf_mj_w = TFMJRefreshTrailWidth;
}
