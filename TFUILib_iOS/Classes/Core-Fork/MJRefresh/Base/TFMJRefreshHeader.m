...
@interface TFMJRefreshHeader() <CAAnimationDelegate>
@property (assign, nonatomic) CGFloat insetTDelta;
@end

@implementation TFMJRefreshHeader
#pragma mark - 构造方法
+ (instancetype)headerWithRefreshingBlock:(TFMJRefreshComponentAction)refreshingBlock
{
    TFMJRefreshHeader *cmp = [[self alloc] init];
...
- (void)prepare
{
    ...
    // 设置高度
    self.tf_mj_h = TFMJRefreshHeaderHeight;
}
...
- (void)placeSubviews
{
    ...
    self.tf_mj_y = - self.tf_mj_h - self.ignoredScrollViewContentInsetTop;
}
...
- (void)resetInset {
    ...
    // sectionheader停留解决
    CGFloat insetT = - self.scrollView.tf_mj_offsetY > _scrollViewOriginalInset.top ? - self.scrollView.tf_mj_offsetY : _scrollViewOriginalInset.top;
    insetT = insetT > self.tf_mj_h + _scrollViewOriginalInset.top ? self.tf_mj_h + _scrollViewOriginalInset.top : insetT;
    self.insetTDelta = _scrollViewOriginalInset.top - insetT;
    // 避免 CollectionView 在使用根据 Autolayout 和 内容自动伸缩 Cell, 刷新时导致的 Layout 异常渲染问题
    if (self.scrollView.tf_mj_insetT != insetT) {
        self.scrollView.tf_mj_insetT = insetT;
    }
}
...
- (void)scrollViewContentOffsetDidChange:(NSDictionary *)change
{
    ...
    // 当前的contentOffset
    CGFloat offsetY = self.scrollView.tf_mj_offsetY;
    // 头部控件刚好出现的offsetY
    ...
    CGFloat pullingPercent = (happenOffsetY - offsetY) / self.tf_mj_h;
    
    if (self.scrollView.isDragging) { // 如果正在拖拽
        self.pullingPercent = pullingPercent;
        if (self.state == TFMJRefreshStateIdle && offsetY < normal2pullingOffsetY) {
            // 转为即将刷新状态
            self.state = TFMJRefreshStatePulling;
        } else if (self.state == TFMJRefreshStatePulling && offsetY >= normal2pullingOffsetY) {
            // 转为普通状态
...
- (void)setState:(TFMJRefreshState)state
{
    ...
    if (state == TFMJRefreshStateIdle) {
        if (oldState != TFMJRefreshStateRefreshing) return;
...
- (void)headerEndingAction {
    ...
    if (!self.isCollectionViewAnimationBug) {
        ...
        [UIView animateWithDuration:self.slowAnimationDuration animations:^{
            self.scrollView.tf_mj_insetT += self.insetTDelta;
    ...
    if (!self.isCollectionViewAnimationBug) {
        [UIView animateWithDuration:self.fastAnimationDuration animations:^{
            if (self.scrollView.panGestureRecognizer.state != UIGestureRecognizerStateCancelled) {
                CGFloat top = self.scrollViewOriginalInset.top + self.tf_mj_h;
                // 增加滚动区域top
                self.scrollView.tf_mj_insetT = top;
                // 设置滚动位置
...
- (void)setIgnoredScrollViewContentInsetTop:(CGFloat)ignoredScrollViewContentInsetTop {
    ...
    self.tf_mj_y = - self.tf_mj_h - _ignoredScrollViewContentInsetTop;
}
