...
- (void)willMoveToSuperview:(UIView *)newSuperview
{
    ...
    if (newSuperview) { // 新的父控件
        if (self.hidden == NO) {
            self.scrollView.tf_mj_insetB += self.tf_mj_h;
        }
        
        // 设置位置
        self.tf_mj_y = _scrollView.tf_mj_contentH;
    } else { // 被移除了
        if (self.hidden == NO) {
            self.scrollView.tf_mj_insetB -= self.tf_mj_h;
        }
...
- (void)scrollViewContentOffsetDidChange:(NSDictionary *)change
{
    ...
    if (self.state != TFMJRefreshStateIdle || !self.automaticallyRefresh || self.tf_mj_y == 0) return;
    
    if (_scrollView.tf_mj_insetT + _scrollView.tf_mj_contentH > _scrollView.tf_mj_h) { // 内容超过一个屏幕
        // 这里的_scrollView.tf_mj_contentH替换掉self.mj_y更为合理
        if (_scrollView.tf_mj_offsetY >= _scrollView.tf_mj_contentH - _scrollView.tf_mj_h + self.tf_mj_h * self.triggerAutomaticallyRefreshPercent + _scrollView.tf_mj_insetB - self.tf_mj_h) {
            // 防止手松开时连续调用
...
- (void)scrollViewPanStateDidChange:(NSDictionary *)change
{
    ...
    switch (panState) {
        ...
        case UIGestureRecognizerStateEnded: {
            if (_scrollView.tf_mj_insetT + _scrollView.tf_mj_contentH <= _scrollView.tf_mj_h) {  // 不够一个屏幕
                if (_scrollView.tf_mj_offsetY >= - _scrollView.tf_mj_insetT) { // 向上拽
                    self.triggerByDrag = YES;
                    [self beginRefreshing];
                }
            } else { // 超出一个屏幕
                if (_scrollView.tf_mj_offsetY >= _scrollView.tf_mj_contentH + _scrollView.tf_mj_insetB - _scrollView.tf_mj_h) {
                    self.triggerByDrag = YES;
                    [self beginRefreshing];
                }
            }
...
- (void)setState:(TFMJRefreshState)state
{
    ...
    if (state == TFMJRefreshStateRefreshing) {
        [self executeRefreshingCallback];
    } else if (state == TFMJRefreshStateNoMoreData || state == TFMJRefreshStateIdle) {
        ...
        if (TFMJRefreshStateRefreshing == oldState) {
            if (self.scrollView.pagingEnabled) {
                ...
                offset.y -= self.scrollView.tf_mj_insetB;
