//
//  NSLayoutConstraint+TFMASDebugAdditions.m
//  Masonry
//
...
#import "NSLayoutConstraint+TFMASDebugAdditions.h"
#import "TFMASConstraint.h"
#import "TFMASLayoutConstraint.h"

@implementation NSLayoutConstraint (TFMASDebugAdditions)
...
+ (NSDictionary *)layoutPriorityDescriptionsByValue {
    ...
    dispatch_once(&once, ^{
        ...
        descriptionMap = @{
            @(TFMASLayoutPriorityDefaultHigh)      : @"high",
            @(TFMASLayoutPriorityDefaultLow)       : @"low",
            @(TFMASLayoutPriorityDefaultMedium)    : @"medium",
            @(TFMASLayoutPriorityRequired)         : @"required",
            @(TFMASLayoutPriorityFittingSizeLevel) : @"fitting size",
        };
#elif TARGET_OS_MAC
        descriptionMap = @{
...
+ (NSString *)descriptionForObject:(id)obj {
    if ([obj respondsToSelector:@selector(tf_mas_key)] && [obj mas_key]) {
...
- (NSString *)description {
    ...
    if (self.priority != TFMASLayoutPriorityRequired) {
