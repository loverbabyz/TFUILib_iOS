...
#define tf_hairlineWidth (1.f/[UIScreen mainScreen].scale)

static void tf_hbd_exchangeImplementations(Class class, SEL originalSelector, SEL swizzledSelector) {
...
@implementation TFHBDNavigationBar
...
- (void)layoutSubviews {
    ...
    self.shadowImageView.frame = CGRectMake(0, CGRectGetHeight(self.shadowImageView.superview.bounds) - tf_hairlineWidth, CGRectGetWidth(self.shadowImageView.superview.bounds), tf_hairlineWidth);
}
...
@implementation UILabel (TFNavigationBarTransition)

- (UIColor *)tf_hbd_specifiedTextColor {
    return objc_getAssociatedObject(self, _cmd);
}

- (void)setTf_hbd_specifiedTextColor:(UIColor *)color {
    objc_setAssociatedObject(self, @selector(tf_hbd_specifiedTextColor), color, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        ...
        tf_hbd_exchangeImplementations(class, @selector(setAttributedText:), @selector(tf_hbd_setAttributedText:));
    });
}

- (void)tf_hbd_setAttributedText:(NSAttributedString *)attributedText {
    if (self.tf_hbd_specifiedTextColor) {
        ...
        [mutableAttributedText addAttributes:@{NSForegroundColorAttributeName: self.tf_hbd_specifiedTextColor} range:NSMakeRange(0, mutableAttributedText.length)];
        attributedText = mutableAttributedText;
    }
    [self tf_hbd_setAttributedText:attributedText];
}
