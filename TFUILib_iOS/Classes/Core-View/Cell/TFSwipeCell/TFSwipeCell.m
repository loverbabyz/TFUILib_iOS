...
@interface TFSwipeCell () <UIScrollViewDelegate,  UIGestureRecognizerDelegate>

@property (nonatomic, weak) UITableView *containingTableView;

@property (nonatomic, strong) UIPanGestureRecognizer *tableViewPanGestureRecognizer;

@property (nonatomic, assign) TFSwipeCellState cellState; // The state of the cell within the scroll view, can be left, right or middle
...
- (CGPoint)contentOffsetForCellState:(TFSwipeCellState)state;
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    if([keyPath isEqualToString:kTableViewPanState] && object == _tableViewPanGestureRecognizer)
    {
        if(_tableViewPanGestureRecognizer.state == UIGestureRecognizerStateBegan)
        {
            ...
            {
                if ([self.delegate respondsToSelector:@selector(swipeTableViewCellShouldHideUtilityButtonsOnSwipe:)])
                {
                    if([self.delegate swipeTableViewCellShouldHideUtilityButtonsOnSwipe:self])
                    {
...
- (void)rightUtilityButtonHandler:(id)sender
{
    ...
    if ([self.delegate respondsToSelector:@selector(swipeTableViewCell:didTriggerRightUtilityButtonWithIndex:)])
    {
        [self.delegate swipeTableViewCell:self didTriggerRightUtilityButtonWithIndex:utilityButtonIndex];
    }
}
...
- (void)leftUtilityButtonHandler:(id)sender
{
    SWUtilityButtonTapGestureRecognizer *utilityButtonTapGestureRecognizer = (SWUtilityButtonTapGestureRecognizer *)sender;
    NSUInteger utilityButtonIndex = utilityButtonTapGestureRecognizer.buttonIndex;
    if ([self.delegate respondsToSelector:@selector(swipeTableViewCell:didTriggerLeftUtilityButtonWithIndex:)])
    {
        [self.delegate swipeTableViewCell:self didTriggerLeftUtilityButtonWithIndex:utilityButtonIndex];
    }
}
...
- (void)hideUtilityButtonsAnimated:(BOOL)animated
{
    if (_cellState != kCellStateCenter)
    {
        ...
        if ([self.delegate respondsToSelector:@selector(swipeTableViewCell:scrollingToState:)])
        {
            [self.delegate swipeTableViewCell:self scrollingToState:kCellStateCenter];
        }
    }
}
...
- (void)showLeftUtilityButtonsAnimated:(BOOL)animated {
    if (_cellState != kCellStateLeft)
    {
        ...
        if ([self.delegate respondsToSelector:@selector(swipeTableViewCell:scrollingToState:)])
        {
            [self.delegate swipeTableViewCell:self scrollingToState:kCellStateLeft];
        }
    }
}
...
- (void)showRightUtilityButtonsAnimated:(BOOL)animated {
    if (_cellState != kCellStateRight)
    {
        ...
        if ([self.delegate respondsToSelector:@selector(swipeTableViewCell:scrollingToState:)])
        {
            [self.delegate swipeTableViewCell:self scrollingToState:kCellStateRight];
        }
    }
}
...
- (void)updateCellState
{
    if(layoutUpdating == NO)
    {
        ...
        {
            TFSwipeCellState cellState = numState.integerValue;
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
    ...
    if ([self.delegate respondsToSelector:@selector(swipeTableViewCell:scrollingToState:)])
    {
        [self.delegate swipeTableViewCell:self scrollingToState:_cellState];
    }
    ...
    if (_cellState != kCellStateCenter)
    {
        if ([self.delegate respondsToSelector:@selector(swipeTableViewCellShouldHideUtilityButtonsOnSwipe:)])
        {
            for (TFSwipeCell *cell in [self.containingTableView visibleCells])
            {
                if (cell != self && [cell isKindOfClass:[TFSwipeCell class]] && [self.delegate swipeTableViewCellShouldHideUtilityButtonsOnSwipe:cell])
                {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    if (scrollView.contentOffset.x > [self leftUtilityButtonsWidth])
    {
        ...
        {
            if (self.delegate && [self.delegate respondsToSelector:@selector(swipeTableViewCell:canSwipeToState:)])
            {
                BOOL shouldScroll = [self.delegate swipeTableViewCell:self canSwipeToState:kCellStateRight];
                if (!shouldScroll)
                {
    ...
    else
    {
        ...
        if ([self leftUtilityButtonsWidth] > 0)
        {
            if (self.delegate && [self.delegate respondsToSelector:@selector(swipeTableViewCell:canSwipeToState:)])
            {
                BOOL shouldScroll = [self.delegate swipeTableViewCell:self canSwipeToState:kCellStateLeft];
                if (!shouldScroll)
                {
                    scrollView.contentOffset = CGPointMake([self leftUtilityButtonsWidth], 0);
                }
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(swipeTableViewCell:didScroll:)]) {
        [self.delegate swipeTableViewCell:self didScroll:scrollView];
    }
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(swipeTableViewCellDidEndScrolling:)]) {
        [self.delegate swipeTableViewCellDidEndScrolling:self];
    }
}
